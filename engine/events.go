...
func (env *Env) saveEventHttp() http.HandlerFunc {
	...
	mapProjectIdToStruct := func(params map[string]string, req *request) (err error) {
		project := params["project"]
		projectId, err := env.DBEngine.ProjectExists(project)
		if err != nil {
			env.Logger.Printf("[error] fetch project by name: %s", err)
			env.Logger.Printf("[debug] requested params: %s", params)
			return
		}
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		req := &request{}
		err := mapProjectIdToStruct(mux.Vars(r), req)
		if err == nil {
		...
		err = env.DBEngine.SaveEvent(req.MetricName, req.ProjectId, req.DurationMs)
		if err == nil {
			saved := struct {
...
//	default:
//		// TODO: check escaping!
//		err = errors.NewEnv(fmt.Sprintf("Range %s is not supported", value))
//	}
//	return
//}
//
